...
policy arrived via a channel notification (WhatsApp message, Telegram message,
...
2. Add `<senderId>` to `allowFrom` (dedupe). Sender IDs are phone numbers
   without the + prefix (e.g., "201234567890" for an Egyptian number).
3. Write back.
...
### `group add <groupJid>` (optional: `--no-mention`, `--allow id1,id2`)
...
- Sender IDs are phone numbers (e.g., "201234567890"). Don't validate format.
